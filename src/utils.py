...
def create_daily_note(vault_path, note_date: Optional[datetime.datetime] = None):
    ...
    if not os.path.exists(daily_path):
        ...
        if template_location and os.path.exists(template_location):
            ...
            sys.stderr.write(
                f"Warning - could not find daily template location: {template_location} using default daily template"
            )
...
def get_daily_template(vault_path):
    ...
    if "template" in content:
        ...
        if rel_path != "":
            ...
            sys.stderr.write(
                "Warning, key `template` exists in dailynote config but is empty str, not using daily template"
            )
            return None
    else:
        sys.stderr.write(
            f"Error, key `template` not in daily-notes.json file {path}, not using a daily template. Content of config: {content}"
        )
        return None
...
def find_header_pos(note_content, header_str):
    ...
    if start_line is None:
        raise ValueError(
            f"Unexpected edge case - Could not find already matched header {header_str}"
        )
...
def insert_text(vault_path, header, inserted_text, create_header_if_missing=False):
    ...
    if start_line is None:
        if create_header_if_missing:
            sys.stderr.write(
                f"Could not find header {header} in daily note, creating it at end of note"
            )
...
def append_to_daily_vault(vault_path, header, message, create_header_if_missing=False):
    new_text = insert_text(
        vault_path, header, message, create_header_if_missing=create_header_if_missing
    )
    daily_path = get_daily_note_path(vault_path)
...
def tree_schema(vault_path, query):
    ...
    headers_index = get_headers_index(
        glob.glob(os.path.join(vault_path, "*"), recursive=True)
    )
    ...
    if path_q:
        ...
        subset_headers = dict(
            (k, headers_index[k]) for k in searched_files if k in headers_index
        )
    else:
    ...
    combined_strings = [
        f"{filename}:{item}"
        for filename, items in subset_headers.items()
        for item in (items if len(items) > 0 else [""])
    ]
        ...
        out.append(
            {
                ...
                "arg": create_obsidian_url(
                    vault_path, fname, heading=(header if len(header) > 1 else None)
                ),
...
def create_obsidian_url(vault_path, relative_path, heading=None, line_num=None):
    ...
    url_scheme = (
        f"obsidian://advanced-uri?vault={vault_name_enc}&filepath={relative_path_enc}"
    )
    if heading:
